...
We have: title of the poem `<titleStmt>`, `<quote>`for citations, the author `<author>`, the person who encodes `<respStmt>`, information about the poem `<publicationStmt>` `<availability>` with attributes`"p"` and `<idno>`, the source `<sourceDesc>` (`<bibl>` containing the title of the collection taken on Gallica).
